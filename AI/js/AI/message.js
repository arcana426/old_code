function respondToUser(comment) {
    ...
    if (comment.includes('こんにちは')) {
        ...
        const expression = comment.replace('を計算して', '').trim(); // 「計算して」の部分を取り除く
        const result = calculateExpression(expression);
        aiResponse = `計算結果は${result}です。`;
    } else {
        aiResponse = 'すみません、よくわかりませんでした。';
    }
    addMessage('AI', aiResponse); // ここでAIの応答を追加します
}
